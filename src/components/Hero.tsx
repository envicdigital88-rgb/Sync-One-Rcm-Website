...
// ── Typewriter config ─────────────────────────────────────────────────────
const CHAR_DELAY  = 0.09;   // 90ms per character — slow & elegant
const START_DELAY = 0.6;

// Each line rendered separately so "Intelligent Billing" gets ONE gradient span
const LINES = [
  { text: 'Maximize',           gold: false },
  { text: 'Healthcare',         gold: false },
  { text: 'Revenue With',       gold: false },
  { text: 'Intelligent Billing', gold: true  },
];

// Build a flat char list just to compute total count & stagger timing
function countChars(lines: typeof LINES) {
  return lines.reduce((acc, l) => acc + l.text.length, 0);
}

// How many characters come before a given line (for delay offset)
function charsBeforeLine(lines: typeof LINES, lineIdx: number) {
  let count = 0;
  for (let i = 0; i < lineIdx; i++) count += lines[i].text.length;
  return count;
}

const TOTAL_CHARS     = countChars(LINES);
const TOTAL_DURATION  = START_DELAY + TOTAL_CHARS * CHAR_DELAY;
    ...
    <>
      ...
      <section id="top" className="relative overflow-hidden">
        <div className="relative flex flex-col justify-end min-h-[640px] sm:min-h-[700px] lg:min-h-screen">
          ...
          {/* ── Hero content ── */}
          <div className="relative z-10 mx-auto w-full max-w-7xl px-4 pb-14 pt-32 sm:px-6 sm:pb-18 lg:px-12 lg:pb-28">
            <div className="max-w-3xl">

              {/* ── Typewriter Headline — line by line ── */}
              <h1 className="font-display text-4xl font-extrabold leading-[1.25] tracking-tight text-white sm:text-5xl lg:text-6xl xl:text-7xl">
                {LINES.map((line, lineIdx) => {
                  const offsetChars = charsBeforeLine(LINES, lineIdx);
                  const chars = line.text.split('');

                  const renderedChars = chars.map((char, charIdx) => {
                    const globalIdx = offsetChars + charIdx;
                    const delay = START_DELAY + globalIdx * CHAR_DELAY;
                  ...
                  // Gold line — wrap ALL chars in a single gradient span so
                  // the gradient spans the full word, not per-character
                  if (line.gold) {
                    return (
                      <React.Fragment key={lineIdx}>
                        <br />
                        <span className="bg-gradient-to-r from-gold-400 via-gold-200 to-gold-500 bg-clip-text text-transparent drop-shadow-lg">
                          {renderedChars}
                        </span>
              ...
              <motion.p
                ...
                transition={{ duration: 1.1, delay: TOTAL_DURATION + 0.25, ease: [0.16, 1, 0.3, 1] }}
              ...
              <motion.div
                ...
                animate={{ opacity: 1, y: 0 }}
                transition={{ duration: 1.0, delay: TOTAL_DURATION + 0.6, ease: 'easeOut' }}
                ...
                <motion.a
                  ...
                  <ChevronRightIcon className="ml-1 size-4 transition-transform duration-200 group-hover:translate-x-1 text-gold-400" aria-hidden="true" />
                </motion.a>
                <motion.a
                  whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.97 }}
                  href="#services"
              ...
              <motion.ul
                ...
                variants={{
                  hidden:  { opacity: 0 },
                  visible: { opacity: 1, transition: { staggerChildren: 0.15, delayChildren: TOTAL_DURATION + 1.0 } },
                }}
            ...
            <div className="relative py-6 md:w-[calc(100%-11rem)]">
              <InfiniteSlider speedOnHover={12} speed={40} gap={80}>
                {allPlatforms.map((platform) =>
                  <span key={platform} className="whitespace-nowrap font-display text-base font-bold tracking-tight text-navy-400">
                    {platform}
                  </span>
                )}
              </InfiniteSlider>
              <div className="pointer-events-none absolute inset-y-0 left-0 w-20 bg-gradient-to-r from-white to-transparent" aria-hidden="true" />
              <div className="pointer-events-none absolute inset-y-0 right-0 w-20 bg-gradient-to-l from-white to-transparent" aria-hidden="true" />
